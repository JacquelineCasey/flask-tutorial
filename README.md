...
- To run locally, use the command `flask run`. This searches for `wsgi.py` or
  `app.py`, and runs that.
...
- The file `Procfile` appears to tell the server what to run on startup; 
  specifically, it runs `gunicorn` and targets `wsgi.py`.
...
- The folder `app` will contain code for the server to use (called by `wsgi.py`),
  and resources like HTML, CSS, and images that are sent.
